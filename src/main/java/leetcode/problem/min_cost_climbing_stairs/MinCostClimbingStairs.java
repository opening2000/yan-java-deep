package leetcode.problem.min_cost_climbing_stairs;
...
class Solution {
    public int minCostClimbingStairs(int[] cost) {
        ...
        if(len == 0){
            ...
            int a = minCost(cost[0], cost, 0);
            int b = minCost(cost[1], cost, 1);
            if(a<b){
                return a;
            }else{
                return b;
    ...
    public int minCost(int p, int[] cost, int start){
        int len = cost.length;
        if(start == (len-1)){
            return p;
        }else if(start == (len-2)){
        ...
        int a = minCost(p + cost[start+1], cost, start+1);
        int b = minCost(p + cost[start+2], cost, start+2);
        if(a<b){
            return a;
        }else{
            return b;
